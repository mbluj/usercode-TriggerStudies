...
720648000*0.00042 //QCD 
  ...
  for(unsigned int i=0; i<5; ++i){
    ...
    for(unsigned int j=0; j<4; ++j){
      ...
      if(j==1)
	cutEta = cut + " && abs(probeEta)<0.9";//barrel
      else if(j==2)
	cutEta = cut + " && abs(probeEta)<1.2 && abs(probeEta)>0.9";//overlap
      else if(j==3)
	cutEta = cut + " && abs(probeEta)<2.1 && abs(probeEta)>1.2";//endcap
  ...
  for(unsigned int iProb=0; iProb<probe.size(); ++iProb){
	...
	if(j==1)
	  cutEta = cut + " && abs(probeEta)<0.9" + " && " + probe[iProb];//barrel
	else if(j==2)
	  cutEta = cut + " && abs(probeEta)<1.2 && abs(probeEta)>0.9" + "&& " + probe[iProb];//overlap
	else if(j==3)
	  cutEta = cut + " && abs(probeEta)<2.1 && abs(probeEta)>1.2" + " && " + probe[iProb];//endcap
	cutEta = "weight*("+cutEta+")";
	if(i!=0){
	  cutEta = cutEta+vtxReWeight;
