...
720648000*0.00042 //QCD 
  ...
  std::string probe1 = "(hltL1sMu16erIsoTau32erORMu16erIsoTau36er>36 && hltL2Tau30eta2p2>35 && hltOverlapFilterIsoMu17L2IsoTau30>35 && hltOverlapFilterIsoMu17MediumIsoPFTau40Reg>0)";//"hltOverlapFilterIsoMu17MediumIsoPFTau40Reg>0"; //"(hltL1sMu16erIsoTau32erORMu16erIsoTau36er>0 && hltL2Tau30eta2p2>0 && hltOverlapFilterIsoMu17L2IsoTau30>0 && hltOverlapFilterIsoMu17MediumIsoPFTau40Reg>0)";//"hltOverlapFilterIsoMu17MediumIsoPFTau40Reg>0";
  std::string probe1MC = "hltL1sMu16erIsoTau36er>0 && hltL2Tau35eta2p2>0 && hltOverlapFilterIsoMu17L2IsoTau35>0 && hltOverlapFilterIsoMu17MediumIsoPFTau40Reg>0";
  std::string probe2 = "(hltL1sMu16erIsoTau32erORMu16erIsoTau36er>36 && hltL2Tau30eta2p2>35 && hltOverlapFilterIsoMu17L2IsoTau30>35)";//"(hltL1sMu16erIsoTau32erORMu16erIsoTau36er>0 && hltL2Tau30eta2p2>0 && hltOverlapFilterIsoMu17L2IsoTau30>0)";
  std::string probe2MC = "hltL1sMu16erIsoTau36er>0 && hltL2Tau35eta2p2>0 && hltOverlapFilterIsoMu17L2IsoTau35>0";
  std::string probe20 = "(hltL1sMu16erIsoTau32erORMu16erIsoTau36er>36)";//"(hltL1sMu16erIsoTau32erORMu16erIsoTau36er>0)";
  std::string probe20MC = "hltL1sMu16erIsoTau36er>0";
  ...
  std::string off   = "muPt>21 && abs(muEta)<2.1 && muIso<0.1 && tauPt>18 && abs(tauEta)<2.1 && muCharge*tauCharge<0 && decayModeFindingNewDMs>0.5 && abs(tauCharge)<1.001 && abs(tauCharge)>0.999 && byCombinedIsolationDeltaBetaCorrRaw3Hits<1.0 && againstElectronVLooseMVA5>0.5 && againstMuonTight3>0.5";
  std::string offSS = "muPt>21 && abs(muEta)<2.1 && muIso<0.1 && tauPt>18 && abs(tauEta)<2.1 && muCharge*tauCharge>0 && decayModeFindingNewDMs>0.5 && abs(tauCharge)<1.001 && abs(tauCharge)>0.999 && byCombinedIsolationDeltaBetaCorrRaw3Hits<1.0 && againstElectronVLooseMVA5>0.5 && againstMuonTight3>0.5";
  ...
  cut = "weight*("+probe1MC+"&&"+selMt+")"+vtxReWeight;
  ...
  cut = "weight*("+probe2MC+"&&"+selMt+")"+vtxReWeight;
  for(unsigned int i=1; i<5; ++i){
    hPtTmp->Reset();
    t[i]->Project("hPtTmp","tauPt",cut.c_str(),"");
    hPtTmp->Scale(scale[i],"width");
  ...
  cut = "weight*("+probe20MC+"&&"+selMt+")"+vtxReWeight;
  for(unsigned int i=1; i<5; ++i){
    hPtTmp->Reset();
    t[i]->Project("hPtTmp","tauPt",cut.c_str(),"");
    hPtTmp->Scale(scale[i],"width");
  ...
  */
  ...
  myErf[0]->SetParameters(40,3.0,1.5,7,0.8);
  /**/
    ...
    */
    ...
    if(fit){
      ...
      */
      ...
      if(trueDYtt){
	myErf[1]->SetParameters(40,3.0,1.5,7,0.8);
	grPtEff[i][1]->Fit("myErf1","M EX0","",18,100);
        //grPtEff[i][1]->Fit("myErf1","M","",18,100);
      }
      myErf[2]->SetParameters(40,3.0,1.5,7,0.8);
