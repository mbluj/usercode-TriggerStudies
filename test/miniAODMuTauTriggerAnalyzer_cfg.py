...
process.muLooseTauMET = process.muLooseTau.clone(
    muons = cms.InputTag("preSelectedMuonsMET"),
    taus = cms.InputTag("preSelectedTausMET"),
    muonTriggers = cms.vstring(  #version number is ignored, so can be replaced by wildcard (*) or dropped
        "HLT_IsoMu16_eta2p1_CaloMET30_v*",
    ),
    muonFilters = cms.vstring(
        "hltL3crIsoL1sMu14erETM30L1f0L2f14QL3f10QL3L3trkIsoFiltered0p09",
    ),
    tauFilters = cms.vstring(
...
process.hltFilter2 = hlt.hltHighLevel.clone(
    TriggerResultsTag = process.muLooseTauMET.bits,
    HLTPaths = process.muLooseTauMET.muonTriggers,
    throw = False
)
...
process.preSelectedMuonsMET = process.preSelectedMuons.clone(
    ...
    cut = cms.string("pt > 17. && abs(eta) < 2.1")
)
...
process.preSelectedTausMET = process.preSelectedTaus.clone(
    cut = cms.string("pt > 18. && abs(eta) < 2.1 && tauID(\'decayModeFinding\')> 0.5 && tauID(\'againstMuonLoose3\') > 0.5 && tauID(\'chargedIsoPtSum\') < 2.0 && leadChargedHadrCand.pt > 30" ) # Loose <2.0, Med <1.0, Tight<0.8
)
...
process.countPreSelMuonsMET = process.countPreSelMuons.clone(
    src = cms.InputTag("preSelectedMuonsMET")
)
...
process.countPreSelTausMET = process.countPreSelTaus.clone(
    src = cms.InputTag("preSelectedTausMET")
)
...
process.p3 = cms.Path(
    process.hltFilter2 +
    process.preSelectedMuonsMET + process.countPreSelMuonsMET +
    process.preSelectedTausMET + process.countPreSelTausMET +
    process.muLooseTauMET
)
